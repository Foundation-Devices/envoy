...
class BleWriteQueue {
    ...
    func enqueue(data: Data) async -> Bool {
        return await withCheckedContinuation { continuation in
            queue.async { [weak self] in
                ...
                print("BleWriteQueue: Enqueued write of size \(data.count)")
    ...
    func onCharacteristicWrite(error: Error?) {
        queue.async { [weak self] in
            ...
            print("BleWriteQueue: onCharacteristicWrite: status=\(success)")
    ...
    // Called when peripheral is ready for more writeWithoutResponse data
    func onPeripheralReady() {
        queue.async { [weak self] in
            guard let self = self else { return }
            
            print("BleWriteQueue: Peripheral ready for next writeWithoutResponse")
    ...
    private func performWrite(data: Data) async -> Bool {
        return await withCheckedContinuation { continuation in
            queue.async { [weak self] in
                ...
                print("BleWriteQueue: performWrite: \(self.writeType), size: \(data.count)")
